...
from rl_attack.experiments.p4_v2de_unified_development import (  # noqa: E402
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run or verify the claim-ineligible P4 v2d/v2e unified-seed development screen"
    )
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    if args.command == "run":
        result = run_p4_v2de_unified_development(args.config, args.output_dir)
    else:
        result = verify_p4_v2de_unified_development(
            args.config,
            args.run,
