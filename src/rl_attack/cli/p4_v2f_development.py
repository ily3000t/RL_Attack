...
from rl_attack.experiments.p4_v2f_development import (  # noqa: E402
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run or verify the claim-ineligible P4-v2f dual-view development screen"
    )
    ...
    verify.add_argument(
        "--full-replay",
        action="store_true",
        help="rerun only the v2f fixed- and own-timing conditions; never rerun golden baselines",
    )
    return parser
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    if args.command == "run":
        result = run_p4_v2f_development(args.config, args.output_dir)
    else:
        result = verify_p4_v2f_development(
            args.config,
            args.run,
