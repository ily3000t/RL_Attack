...
from rl_attack.experiments.p4_v2e_postgate_diagnostic import (  # noqa: E402
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run or verify the P4-v2e post-gate exploratory diagnostic"
    )
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    if args.command == "run":
        result = run_p4_v2e_postgate_diagnostic(args.config, args.output_dir)
    else:
        result = verify_p4_v2e_postgate_diagnostic(
            args.config,
            args.run,
